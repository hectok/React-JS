...
export default function Login(props) {
   ...
   *
   * @param {}
   * @returns Si el usuario se registra correctamente, iniciamos su sesión y lo redirigimos a la página principal (Store)
   */
  let createUser = () => {
    let path = `login`;
    history.push(path);
  ...
  /**
   *
   * @param {event} event.usuario
   * @param {event} event.password
   * @returns Recuperamos del local storage la combinación de "key" + el nombre de usuario introducido por el cliente.
  ...
  let signIn = (event) => {
    let retrievedObject = JSON.parse(
      localStorage.getItem("usuario_" + event.usuario)
    );

    if (retrievedObject) {
      if (
        event.usuario === retrievedObject.usuario &&
        event.password === retrievedObject.password
      ) {
        setSession(true);
        let path = `store`;
        history.push(path);
                ...
                >
                  {({ values, touched, errors }) => (
                    <Form className="form-horizontal">
                      ...
                      <div
                        className={[
                          touched.usuario && errors.usuario && "errorInput",
                          "form-group",
                        ].join(" ")}
                      >
                        <Field
